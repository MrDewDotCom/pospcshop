...
| See revenue totals (net sales, refunds, sales chart) — `revenue.view`, Q13              |  ✅   |      ❌ (each sale/return they handle: ✅)       |
...
| POST           | /stock/adjustments 🔒                         | reason required; serials required for serial products. One `AJ…` doc number groups the lines (no table: the ledger rows carry `ref_type='adjustment'` + the number). Removed serial units → `written_off`; found units come back on their row |
...
| POST           | /sales                             | confirm checkout: `{ items: [{ productId, qty, serialItemIds }], customerId?, note?, payment: { method, receivedSatang }, expectedTotalSatang }` → sale + payment + stock out in one transaction (P25)                          |
